...
public class SubirFotosActivity extends ActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			cSql = "select count(*) as reg from customssn where not subida and length(trim(foto)) > 0 ";
	            	...
	            	if (nMax>0) {
		            	...
		            	new Thread(new Runnable() {
		                    public void run() {
		                        //while (nReg < nMax) {
	                        	//nReg = doWork();
	    		            	cSql = "select * from customssn where not subida and length(trim(foto)) > 0 order by id ";
	    		    			rs = DatabasePostgres.getRS(cSql);
	    		    			...
	    		    			try {
		    			            while (!(rs.isAfterLast())) {
		    			            	...
		    			            	if (lCapFoto>0) {
		    			            		cSql = "update customssn set subida = true, fecha_subida = now() where id = " + rs.getInt("id");
		    			            		mx.sounds.customssn.data.DatabasePostgres.execQuery(cSql);
		    			            	}			            	
		    			            	rs.next();
		    			            }
		    			            rs.close();
